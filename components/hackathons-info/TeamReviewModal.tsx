...
import { Review, Team, TeamMember } from "./JudgeDashboard";
...
interface TeamReviewModalProps {
    ...
    onClose: () => void;
    team: Team | null,
    onSubmitReview: (reviewData: { score: number; comments: string }) => Promise<void>;
}
...
const TeamReviewModal: React.FC<TeamReviewModalProps> = ({
    ...
    const getReviewData = () => {
        ...
        const res = fetch(`/api/hackathons/${team?._id}/review-data`).then(res => res.json());
        console.log(res);
    }

    useEffect(() => {
        if(team && isOpen) getReviewData()
    }, [team, isOpen])


    const [score, setScore] = useState<number>(team?.reviews?.score || 0);
    const [comments, setComments] = useState<string>(team?.reviews?.comments || "");
    const [submitting, setSubmitting] = useState(false);

    const handleSubmit = async (e: React.FormEvent) => {
        e.preventDefault();
        setSubmitting(true);
        await onSubmitReview({ score, comments });
        setSubmitting(false);
        onClose();
    };
                    ...
                    <div className="flex-1 overflow-y-auto p-6 grid grid-cols-1 md:grid-cols-2 gap-8">
                        <div className="space-y-6">
                            ...
                            <div className="bg-blue-50 p-4 rounded-lg border border-blue-100">
                                <h4 className="font-medium text-blue-900 mb-2 text-sm">Team Members</h4>
                                <div className="flex flex-wrap gap-2">
                                    {team?.members.map((member: TeamMember) => (
                                        <span key={member.userId} className="bg-white px-3 py-1 rounded-full text-xs font-medium text-blue-700 shadow-sm border border-blue-100">
                                            {member.name}
                                        </span>
                                    ))}
                                </div>
                        ...
                        <div className="bg-gray-50 rounded-xl p-6 border border-gray-100 h-fit">
                            <div className="flex items-center gap-2 mb-6">
                                ...
                                <h3 className="text-lg font-semibold text-gray-900">Judge&apos;s Review</h3>
                            </div>
                            ...
                            <form onSubmit={handleSubmit} className="space-y-6">
                                <div>
                                    ...
                                    <input
                                        ...
                                        value={score}
                                        onChange={(e) => setScore(Number(e.target.value))}
                                        className="w-full mt-2 h-4 bg-gray-200 rounded-lg cursor-pointer accent-blue-600"
                                    />
                                </div>
